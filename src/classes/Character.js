class Character {
  constructor(scene, config) {
    ...
    if (!this.spawnPoint || typeof this.spawnPoint.x === 'undefined' || typeof this.spawnPoint.y === 'undefined') {
      console.warn(`Invalid spawn point for ${this.name}, using default position`);
    ...
    // Create animations before starting roaming
    this.createAnimations();

    this.scene.physics.add.collider(this.sprite, config.worldLayer);
    this.createNameLabel();
    
    // Start roaming after animations are created
    if (this.isRoaming) {
        this.startRoaming();
    }
  }
  ...
  createAnimations() {
    if (!this.atlas) return; // Skip if no atlas is defined
    
    const anims = this.scene.anims;
    const directions = ['left', 'right', 'front', 'back'];
    
    // Create static frames if they don't exist
    directions.forEach(direction => {
        const frameKey = `${this.id}-${direction}`;
        if (!anims.get(frameKey)) {
            anims.create({
                key: frameKey,
                frames: [{ key: this.atlas, frame: frameKey }]
            });
        }
    });
    ...
    directions.forEach(direction => {
        ...
        if (!anims.get(key)) {
            try {
                ...
                anims.create({
                    key: key,
                    frames: [{ key: this.atlas, frame: `${this.id}-${direction}` }]
                });
            }
        }
    });
